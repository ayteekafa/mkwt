(function(){
  ...
  MKWT.setStatus = function(targets, msg, ok){
    ...
    arr.forEach((target)=>{
      ...
      if (!el) return;
      const base = Array.from(el.classList).filter(c => c !== 'ok' && c !== 'bad').join(' ') || 'muted';
      el.className = base + (msg ? (' ' + (ok ? 'ok' : 'bad')) : '');
      el.textContent = msg || '';
    });
  };
  ...
  MKWT.bindInfoOverlay = function(options){
    ...
    function closeInfo(){ const overlay = MKWT.$(cfg.overlayId); if (overlay){ overlay.hidden = true; overlay.style.display = 'none'; } }
    document.addEventListener('DOMContentLoaded', ()=>{
      MKWT.$(cfg.closeId)?.addEventListener('click', closeInfo);
      document.querySelectorAll(cfg.triggerSelector).forEach((btn)=>{
        btn.addEventListener('click', (e)=>{ e.preventDefault(); e.stopPropagation(); openInfo(btn.dataset.info || btn.dataset.chart || ''); });
      });
